...
namespace cost_estimating
{
    ...
    public class Complex
    {
        ...
        public static Complex operator *(double c,Complex C)
        {
            //c*(a+b*i)=c*a+c*b*i
            return new Complex(c*C.real,c*C.image);
        }
        ...
        public override string ToString()
        {
            ...
            if (this.image == 0)
            {
                return string.Format("{0}", Math.Round(this.real, 3, MidpointRounding.AwayFromZero));//中国式四舍五入
            }
            else if (this.image < 0)
            {
                return string.Format("{0} - j{1}", Math.Round(this.real, 3, MidpointRounding.AwayFromZero),
                                               (0-Math.Round(this.image, 3, MidpointRounding.AwayFromZero)));//中国式四舍五入
            }
            else 
            {
                return string.Format("{0} + j{1}", Math.Round(this.real, 3, MidpointRounding.AwayFromZero),
                                               Math.Round(this.image, 3, MidpointRounding.AwayFromZero));//中国式四舍五入
            }
